...
import PopupCapNhatNguoiDung from "./UpdateUserPopup";
import PopupThemNguoiDung from "./AddUserPopup";
import { getUsersPhanTrang } from "../../../stores/user/GETUsersDivPageReducers";
import { getUsersID } from "../../../stores/user/GETUsersIdReducers";
import {
  deleteUser,
  deleteUserActions,
} from "../../../stores/user/DELETEUsersReducers";
...
const QuanLyNguoiDung: React.FC = () => {
  ...
  const { messageDeleteUser, errMessageDeleteUser } = useSelector(
    (state: RootState) => state.deleteUserReducer
  );
  const { contentPostUser } = useSelector(
    (state: RootState) => state.postUserReducer
  );
  ...
  useEffect(() => {
    dispatch(
      getUsersPhanTrang(
        `pageIndex=${search.get("pageIndex")}&pageSize=10&keyword=${search.get(
          "keyword"
        )}`
      )
    );
    ...
    <Container className="bg-white m-3 p-3 QuanLyNguoiDung">
        ...
        <tbody className="block h-[450px] overflow-auto">
          {contentGetUsers?.data.map((nguoiDung, i) => (
            ...
            >
              ...
              <td
                className="w-28 break-words cursor-pointer"
                onClick={() => {
              ...
              <td className="w-20 break-words">
                {nguoiDung.gender === true ? "Nam" : "Nữ"}
              </td>
              <td className="w-24 break-words">{nguoiDung.role}</td>
              <td className="flex-1 space-x-1">
                <button
                  onClick={() =>
                    navigate(`/admin/quanLyDatPhong/${nguoiDung.id}`)
                  }
                  className="bg-blue-800 p-1 rounded-md shadow text-white hover:bg-blue-500"
                >
                  CT
                </button>
                <button
                ...
                <button
                  onClick={() => dispatch(deleteUser(nguoiDung.id.toString()))}
      ...
      <div>
        <Pagination
          ...
          total={contentGetUsers?.totalRow}
      ...
      {messageDeleteUser || errMessageDeleteUser ? (
        <div className="fixed top-0 bottom-0 left-0 right-0 bg-black/40">
          <div className="w-80 h-40 bg-white mx-auto mt-40 shadow flex flex-col justify-center items-center">
            ...
            <button
              onClick={() =>
                dispatch(deleteUserActions.removeMessageDeleteUser(""))
              }
