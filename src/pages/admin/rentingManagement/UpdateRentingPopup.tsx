...
import {
  putDatPhongID,
  putDatPhongIDActions,
} from "../../../stores/rentRoom/PUTRentRoomByIdReducers";
import {
  getPhongThueID,
  getPhongThueIDActions,
} from "../../../stores/roomForRent/roomForRentId";
import {
  getPhongThueTheoViTri,
  getPhongThueTheoViTriActions,
} from "../../../stores/roomForRent/roomForRentReducers";
...
const PopupCapNhatDatPhong: React.FC<Props> = (props) => {
  ...
  const { contentGetDatPhongID } = useSelector(
    (state: RootState) => state.getDatPhongIDReducer
  );
  const { contentPutDatPhong, errContentPutDatPhong } = useSelector(
    (state: RootState) => state.putDatPhongIDReducer
  );
  const phongThue = props.contentGetPhongThueAll?.find(
    (phongThue) => phongThue.id === contentGetDatPhongID?.maPhong
  );
  const viTriID = phongThue?.maViTri.toString();
  const dSPhongThueTheoViTri = props.contentGetPhongThueAll?.reduce<
    PhongThue[]
  >((dSPhongThueTheoViTri, phongThue) => {
  ...
  useEffect(() => {
    ...
    if (contentGetDatPhongID) {
      setNgayDen(
        new Date(
          moment(contentGetDatPhongID.ngayDen).format("YYYY-MM-DD")
        ).getTime()
      );
      setNgayDi(
        new Date(
          moment(contentGetDatPhongID.ngayDi).format("YYYY-MM-DD")
        ).getTime()
      );
    }
      ...
      <div className="text-right">
        <button
          onClick={() => {
            ...
            dispatch(
              getPhongThueTheoViTriActions.removecontentGetPhongThueTheoViTri(
                ""
              )
            );
      ...
      <form
        onSubmit={handleSubmit((data) => {
          ...
          if (data.ngayDen > data.ngayDi) {
            ...
            dispatch(
              putDatPhongID({
                id: contentGetDatPhongID?.id.toString(),
                data: data,
              })
            );
          }
      ...
      >
        <div className="mb-2">
          ...
          <select
            ...
            onChange={(e) => dispatch(getPhongThueTheoViTri(e.target.value))}
            onClick={() => {
              dispatch(
                getPhongThueTheoViTriActions.removecontentGetPhongThueTheoViTri(
                  ""
        ...
        <div className="mb-2">
          ...
          <select
            ...
            {...register("maPhong")}
            className="w-full border border-gray-500 focus:outline-none px-2 py-[2px] focus:border-blue-600 cursor-pointer"
            onChange={(e) => dispatch(getPhongThueID(e.target.value))}
          >
            {contentGetPhongThueTheoViTri ? (
              <>
                <option className="hidden"></option>
                {contentGetPhongThueTheoViTri?.map((phongThue, i) => (
                  <option key={i} value={phongThue.id}>
                    {phongThue.tenPhong}
                  </option>
                ))}
              </>
        ...
        <div className="mb-2">
          ...
          >
            {contentGetPhongThueID
              ? soKhach(contentGetPhongThueID?.khach)
              : phongThue
              ? soKhach(phongThue?.khach)
              : ""}
          </select>
        </div>
        ...
        {ngayDi >= ngayDen && ngayDen !== 0 ? (
          <div className="flex justify-between mb-3 text-base font-semibold bg-gray-300 p-2">
            <p className="m-0">
              Tổng tiền: {contentGetPhongThueID?.giaTien || phongThue?.giaTien}
              $/ngày <AiOutlineClose className="inline-block mb-1" />{" "}
              {(ngayDi - ngayDen) / (24 * 3600 * 1000) + 1} ngày
            </p>
            <p className="m-0">
              {contentGetPhongThueID?.giaTien
                ? contentGetPhongThueID?.giaTien *
                  ((ngayDi - ngayDen) / (24 * 3600 * 1000) + 1)
                : phongThue?.giaTien
                ? phongThue?.giaTien *
                  ((ngayDi - ngayDen) / (24 * 3600 * 1000) + 1)
                : ""}
