...
import PopupThemViTri from "./AddPositionPopup";
import PopupCapNhatViTri from "./UpdatePositionPopup";
import { getViTriPhanTrang } from "../../../stores/position/GETPositionsDivPageReducers";
import { getViTriID } from "../../../stores/position/GETPositionsByIdReducers";
import {
  deleteViTri,
  deleteViTriActions,
} from "../../../stores/position/DELETEPositionsReducers";

const QuanLyThongTinViTri: React.FC = () => {
  const { register, handleSubmit, reset } = useForm();
  const dispatch = useDispatch<any>();
  const { contentGetViTri } = useSelector(
    (state: RootState) => state.getViTriPhanTrangReducer
  );
  const { messageDeleteViTri, errMessageDeleteViTri } = useSelector(
    (state: RootState) => state.deleteViTriReducer
  );
  const { contentPostViTri } = useSelector(
    (state: RootState) => state.postViTriReducer
  );
  const { contentPutViTri } = useSelector(
    (state: RootState) => state.putViTriIDReducer
  );
  ...
  return (
    <Container className="bg-white m-3 p-3 QuanLyThongTinViTri">
        ...
        <tbody className="block h-[450px] overflow-auto">
          {contentGetViTri?.data.map((viTri, i) => (
            ...
            >
              ...
              <td className="w-40 break-words">
                <img src={viTri.hinhAnh} alt="..." width={130} />
              </td>
              ...
              <td className="flex-1 space-x-2">
                ...
                <button
                  onClick={() => dispatch(deleteViTri(viTri.id.toString()))}
      ...
      <div>
        <Pagination
          defaultPageSize={5}
          total={contentGetViTri?.totalRow}
      ...
      {messageDeleteViTri || errMessageDeleteViTri ? (
        <div className="fixed top-0 bottom-0 left-0 right-0 bg-black/40">
          <div className="w-80 h-40 bg-white mx-auto mt-40 shadow flex flex-col justify-center items-center">
            ...
            <button
              onClick={() =>
                dispatch(deleteViTriActions.removeMessageDeleteViTri(""))
              }
