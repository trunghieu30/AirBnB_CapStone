...
import {
  putViTriID,
  putViTriIDActions,
} from "../../../stores/position/PUTPositionsByIdReducers";
...
const PopupCapNhatViTri: React.FC<Props> = (props) => {
  ...
  const { contentGetViTriID } = useSelector(
    (state: RootState) => state.getViTriIDReducer
  );
  const { contentPutViTri, errContentPutViTri } = useSelector(
    ...
    <Container className="PopupCapNhatViTri w-1/3 py-2 px-5 bg-white mx-auto mt-10 shadow">
      ...
      <div className="h-14">
        <p className="text-xl text-center font-bold m-0">
          Cập nhật vị trí {contentGetViTriID?.id}
        </p>
        {contentPutViTri ? (
          ...
          <p className="text-center text-red-500 m-0">{errContentPutViTri}</p>
        )}
      </div>
      <form
        onSubmit={handleSubmit((data) => {
          delete data.image;
          dispatch(
            putViTriID({ id: contentGetViTriID?.id.toString(), data: data })
          );
        })}
      >
        <div className="mb-2">
        ...
        <div className="mb-2">
          ...
          <img
            src={urlHinhAnh || contentGetViTriID?.hinhAnh}
            alt="..."
