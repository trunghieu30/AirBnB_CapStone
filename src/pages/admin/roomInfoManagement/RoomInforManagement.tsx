...
import { getPhongThuePhanTrang } from "../../../stores/roomForRent/GETRoomForRentDivPageReducers";
import {
  deletePhongThue,
  deletePhongThueActions,
} from "../../../stores/roomForRent/DELETERoomForRentReducers";
import PopupThemPhongThue from "./AddRoomForRentPopup";
import PopupCapNhatPhongThue from "./UpdateRoomForRent";
...
const QuanLyThongTinPhong: React.FC = () => {
  ...
  const { contentGetPhongThue } = useSelector(
    (state: RootState) => state.getPhongThuePhanTrangReducer
  );
  const { messageDeletePhongThue, errMessageDeletePhongThue } = useSelector(
    (state: RootState) => state.deletePhongThueReducer
  );
  const { contentPostPhongThue } = useSelector(
    (state: RootState) => state.postPhongThueReducer
  );
  const { contentPutPhongThue } = useSelector(
  ...
  useEffect(() => {
    dispatch(
      getPhongThuePhanTrang(
        `pageIndex=${search.get("pageIndex")}&pageSize=5&keyword=${search.get(
          "keyword"
        )}`
      )
    );
    ...
    search,
    messageDeletePhongThue,
    contentPostPhongThue,
    contentPutPhongThue,
  ]);
  ...
  return (
    <Container className="bg-white m-3 p-3 QuanLyThongTinPhong">
      ...
      <button
        onClick={() => setDisplay("")}
        className="mb-3 py-1 px-3 font-semibold border border-amber-500 text-amber-500 hover:bg-amber-500 hover:text-white"
      >
        Thêm phòng thuê
      </button>
      <form
        onSubmit={handleSubmit((data) => {
          setSearch({ keyword: data.keyword, pageIndex: "1" });
      ...
      >
        ...
        />
        <button className=" bg-amber-800 text-white px-3 hover:bg-amber-500">
          search
        </button>
      </form>
      <table className="w-full mb-3">
        ...
        <tbody className="block h-[450px] overflow-auto">
          {contentGetPhongThue?.data.map((phongThue, i) => (
            <tr
              key={i}
              className="border-b text-left flex items-center pl-3 py-2"
            >
              <td className="w-10">{phongThue.id}</td>
              <td className="w-24">
                <img src={phongThue.hinhAnh} alt="" className="w-11/12" />
              </td>
              <td className="w-24 break-work">{phongThue.tenPhong}</td>
              <td className="w-[520px] break-work">
                <div>
                  <span>Số khách: {phongThue.khach}</span> -{" "}
                  <span>Phòng ngủ: {phongThue.phongNgu}</span> -{" "}
                  <span>Số giường: {phongThue.giuong}</span> -{" "}
                  <span>Phòng tắm: {phongThue.phongTam}</span>
                </div>
                <div>
              ...
              <td className="flex-1 space-x-2">
                <button
                  ...
                  className="bg-green-800 p-1 rounded-md shadow text-white hover:bg-green-500"
                >
                  sửa
                </button>
                <button
                  onClick={() =>
                    dispatch(deletePhongThue(phongThue.id.toString()))
                  }
                  className="bg-red-800 p-1 rounded-md shadow text-white hover:bg-red-500"
                >
                  Xoá
                </button>
              </td>
      ...
      <div>
        <Pagination
          defaultPageSize={5}
          total={contentGetPhongThue?.totalRow}
          current={Number(search.get("pageIndex"))}
        ...
        <div className="fixed top-0 bottom-0 left-0 right-0 bg-black/40">
          <div className="w-80 h-40 bg-white mx-auto mt-40 shadow flex flex-col justify-center items-center">
            ...
            <button
              onClick={() =>
                dispatch(
                  deletePhongThueActions.removeMessageDeletePhongThue("")
                )
              }
      ...
      <div
        className={`${display} fixed top-0 bottom-0 left-0 right-0 bg-black/50`}
      >
        <PopupThemPhongThue
      ...
      <div
        className={`${displayUpdate} fixed top-0 bottom-0 left-0 right-0 bg-black/50`}
      >
