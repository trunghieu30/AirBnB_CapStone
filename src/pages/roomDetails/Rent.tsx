  ...
  return (
    <div>
      <div className="flex justify-between items-end">
        <p>
          <span className="text-2xl font-semibold text-amber-800">
            {props.contentGetPhongThueID?.giaTien}$
          </span>{" "}
          <span className="text-base">/ ngày</span>
        </p>
        <p>
      ...
      >
        <div className="mb-5 border border-gray-300 rounded-xl">
          <div className="flex">
            <div className="w-1/2 border-r border-gray-300 px-3 py-2">
              <p className="m-0 font-semibold pr-2 text-amber-800">
                Nhận phòng
              </p>
              <input
                required
                ...
                type="date"
                className="w-full text-base flex-1 focus:outline-none py-[2px]"
              />
            </div>
          ...
          <div className="px-3 py-2 border-t border-gray-300">
            ...
            <select
              ...
              className="w-full text-base focus:outline-none py-[2px] cursor-pointer"
            >
              <option value=""></option>
              {props.contentGetPhongThueID?.khach
                ? soKhach(props.contentGetPhongThueID?.khach)
                : ""}
            </select>
          </div>
        </div>
        <div>
          <button
            ...
            className="w-full px-7 py-3 bg-amber-800 text-white font-medium text-sm uppercase rounded shadow-md hover:bg-amber-500 transition duration-300"
          >
            Đặt phòng
          </button>
        </div>
      ...
      <div className="flex justify-between pt-3 border-t text-xl">
        ...
        {ngayDi >= ngayDen && ngayDen !== 0 ? (
          props.contentGetPhongThueID?.giaTien ? (
            <p className="font-bold m-0 text-amber-800">
              {props.contentGetPhongThueID?.giaTien *
                ((ngayDi - ngayDen) / (24 * 3600 * 1000) + 1)}
              $
            </p>
