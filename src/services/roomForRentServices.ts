...
export const phongThue = {
  ...
  getPhongThuePhanTrang: (data: string) => {
    return api.get<HttpResponse<ContentGetPhongThuePhanTrang<PhongThue[]>>>(
      `/api/phong-thue/phan-trang-tim-kiem?${data}`
    );
  },
  ...
  getPhongThueTheoViTri: (data: string) => {
    return api.get<HttpResponse<PhongThue[]>>(
      `/api/phong-thue/lay-phong-theo-vi-tri?maViTri=${data}`
    );
  },
};
